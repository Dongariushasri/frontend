...
export class AdminService {
  ...
  private apiUrl = 'https://revtask-mgmt.azurewebsites.net/api';


  constructor(private http: HttpClient) { }

  statusOfProject:string="Pending";
  ...
  login(email: string, password: string): Observable<{ role: string, id: number }> {
    return this.http.post<{ role: string, id: number }>(`${this.apiUrl}/admin/alogin`, { email, password }).pipe(
      map(response => {
       ...
       // console.log(localStorage.getItem('TeamMember'))
        return response;
      })
    );
  }
...
updateTaskStatus(id: number, status: string): Observable<Object> {
  return this.http.put(`${this.apiUrl}/teammember/uptasks/${id}`, status);
}
